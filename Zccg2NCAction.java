...
public class Zccg2NCAction implements Action{
	...
	public String execute(RequestInfo requestInfo) {
		try {
            log.info("进入《资产采购付款申请流程》流程结束传值给NC方法...");
            requestid = requestInfo.getRequestid();
            billid=requestInfo.getRequestManager().getBillid()+"";
            Map<String, String> zccgMainMap = TableDataUtil.getMainMap(log, requestInfo);// 主表
            //List<Map<String,String>> listMap1 = TableDataUtil.getDtListMap(log, requestInfo, 0);// 明细表1
            // 调用NC-WebService接口
            String xml = getZccgXml(zccgMainMap);// 接口参数
            log.info("xml:" + xml);
	 ...
	 * 获取费用报销XML
	 * 
	 * @param zccgMainMap 《资产采购付款申请流程》主表数据
	 * @return NC规定的接口所需数据
	 * @author tengfei.ye@weaver.cn
	 * @version 2019/7/18 22:19
	 */
	private String getZccgXml(Map<String, String> zccgMainMap) {
		...
		xmlSb.append("<dbilldate>").append(zccgMainMap.get("sqrq")).append("</dbilldate>");// 凭证日期
		xmlSb.append("<billmaker>").append("hlj").append("</billmaker>");// 制单人编码 暂时只能使用hlj
		if (zccgMainMap.get("xgfj") != "") {
			int fjsl = zccgMainMap.get("xgfj").split(",").length;// 附件数量
		...
		xmlSb.append("<explanation>").append(zccgMainMap.get("zy")).append("</explanation>");// 摘要
		xmlSb.append("<accsubjcode>").append(zccgMainMap.get("jfkm")).append("</accsubjcode>");// 科目编码
		xmlSb.append("<pk_currtype>").append("CNY").append("</pk_currtype>");// 币种
		xmlSb.append("<excrate2>").append("1.000000").append("</excrate2>");// 折本汇率
		xmlSb.append("<debitamount>").append(zccgMainMap.get("bcfkje")).append("</debitamount>");// 借方金额
		xmlSb.append("<creditamount>").append("0.00").append("</creditamount>");// 贷方金额
		xmlSb.append("<ass>");
		if(zccgMainMap.get("fphm").equals("")) {//如果无发票
			if (!zccgMainMap.get("fzhsry").equals("")) {
				xmlSb.append("<item>");
				xmlSb.append("<pk_Checktype>").append("人员档案").append("</pk_Checktype>");// 人员辅助核算名称
				xmlSb.append("<pk_Checkvalue>").append(zccgMainMap.get("fzhsry")).append("</pk_Checkvalue>");// 人员辅助核算编码
				xmlSb.append("</item>");
			}
			xmlSb.append("<item>");
